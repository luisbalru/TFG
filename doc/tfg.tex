...
\textbf{Teorema 2.} La transformada de Fourier es una aplicación continua y biyectiva del espacio de Schwartz en sí mismo. Para $u \in S(\mathbb{R}^d)$, tenemos la fórmula de inversión
	...
	$$(F^{-1}Fu)(x) = \check{\hat{u}}(x)= \frac{1}{(2\pi)^{d/2}} \int_{\mathbb{R}^d} \hat{u}(\xi) e^{ix\xi} d\xi = u(x)$$
...
Por propiedades de conjugación de la transformada de Fourier, tenemos que $\check{u} = D_{-id} \hat{u}$ y sustituyendo $\hat{u}$ por $u$, obtenemos:

$$\check{\hat{u}} = D_{-id} \hat{\hat{u}} = u$$
...
$$(u,v)_2 = (Fu,Fv)_2$$
que se conoce como la fórmula de Plancharel.
\end{itemize}
\newpage
